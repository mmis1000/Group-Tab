...
define(function(require) {
    ...
    var initialize = async function() {
        ...
        if (data.trie != null
            ...
            && data.min_freq != null) {
            try {
                trie = JSON.parse(data.trie)
                FREQ = JSON.parse(data.FREQ)
                total = JSON.parse(data.total)
                min_freq = JSON.parse(data.min_freq)
                
                console.log("Using Trie from cache...")
                initialized = true;
                return
        ...
        if (trie == null || FREQ == null || total == null || min_freq == null) {
            ...
            await browser.storage.local.set({
                ...
                min_freq: JSON.stringify(min_freq)
            })
        }
    ...
    let initializeCB = ()=>{
        window.jiebaInstance = {
            cut
        }
        
        if (window.jiebaQuerys.length > 0) {
            window.jiebaQuerys.forEach((q, index)=>{
                // there must be some timeout, or it will hang
                setTimeout(()=>{
                    q.cb(cut(q.str))
                }, index * 50)
            })
